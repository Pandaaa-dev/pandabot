...
module.exports= {
    ...
    async run(connection, client, emoji){
        ...
        const AuditLog = await guild.fetchAuditLogs({limit: 1, type: 'EMOJI_CREATE'})
        const requiredLog = AuditLog.entries.first()
        const member = guild.member(requiredLog.executor)
        await loggingChannel.send(loggingEmbed(client, `Emoji Added`, guild, `${emoji} was added by ${member}`))
    }

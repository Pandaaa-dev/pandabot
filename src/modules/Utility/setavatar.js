...
module.exports = {
    ...
    usage(prefix){
        ...
        const single = `\`${prefix}${this.name.toLowerCase()}  <...avatarlink>\``
    ...
    async execute( message, args, text, client){
        ...
        if(args[0].startsWith('https://')){
            ...
            client.user.setAvatar(args[0]).then(res => {
                ...
                if( e.code == 50035){
                  return  message.channel.send(basicEmbed(client,message,args,text,`Changing Avatar too fast!`, '😠', 'You are changing avatars too fast! Please try again later.'))
                }
            })
        } else {
            return  message.channel.send(basicEmbed(client,message,args,text,`Wrong Url!`, '😠', 'Please copy the image link and not the link itself. Also, we do not support images from \'http://\'(unprotected) sites! '))
        }
    }
}